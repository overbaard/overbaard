...
import org.overbaard.jira.api.adapter.JiraApiAdapterAbstractFactory;
 ...
 *     construct the needed {@code OverbaardIssueEvent} instances.</li>
 ...
 * </ol>
 * <p/>
 * The {@code OverbaardIssueEvent} instances created in the first step are used to update our board caches when receiving
 ...
 * <p/>
 * However, if an issue is only re-ranked via Overbård or Jira Agile's board, the {@code ReindexIssuesCompletedEvent} and
 * the {@code IssueEvent} can come in any order. However a 'pure' re-rank is initiated by a {@code LexoRankBalanceEvent},
 * so we use OverbaardEventWrapper to track that we have all the needed information.
 *
 *
 *
 * @author Kabir Khan
 */
@Named("overbaardIssueEventListener")
public class OverbaardIssueEventListener implements InitializingBean, DisposableBean {
    private static final Logger log = LoggerFactory.getLogger(OverbaardIssueEventListener.class);
    ...
    private final WrappedThreadLocal<OverbaardEventWrapper> delayedEvents = new WrappedThreadLocal<>();
     ...
     */
    @Autowired
    public OverbaardIssueEventListener(EventPublisher eventPublisher,
        ...
        System.out.println("----> Issue Event Listener!!!");
        JiraApiAdapterFactory jiraApiAdapterFactory = JiraApiAdapterAbstractFactory.getAdapterFactory();
        TestInterface testInterface = jiraApiAdapterFactory.getTestInterface();
        testInterface.sample();
    }
    ...
    public void onRankEvent(LexoRankChangeEvent event) {
        OverbaardLogger.LOGGER.debug("LexoRankChangeEvent on thread {}", Thread.currentThread().getName());

        OverbaardEventWrapper wrapper = new OverbaardEventWrapper(true);
    ...
    public void onEvent(ReindexIssuesCompletedEvent event) throws IndexException {
        OverbaardLogger.LOGGER.debug("ReindexIssuesCompletedEvent on thread {}", Thread.currentThread().getName());

        OverbaardEventWrapper delayedEvent = delayedEvents.get();
        if (delayedEvent != null) {
            delayedEvent.reindexed = true;
            if (delayedEvent.isComplete()) {
                try {
                    OverbaardLogger.LOGGER.debug("Handle delayed event {}", delayedEvent.issueEvent.getIssueKey());
    ...
    public void onIssueEvent(IssueEvent issueEvent) throws IndexException {
        OverbaardLogger.LOGGER.debug("IssueEvent {} on thread {}", issueEvent, Thread.currentThread().getName());
    ...
    private void onDeleteEvent(IssueEvent issueEvent) throws IndexException {
        ...
        final OverbaardIssueEvent event = OverbaardIssueEvent.createDeleteEvent(issue.getKey(), issue.getProjectObject().getKey());
        passEventToBoardManagerOrDelay(event);
    }
    ...
    private void onMoveEvent(IssueEvent issueEvent) throws IndexException {
        ...
        if (isAffectedProject(oldProjectCode)) {
            final OverbaardIssueEvent event = OverbaardIssueEvent.createDeleteEvent(oldIssueKey, oldProjectCode);
            passEventToBoardManagerOrDelay(event);
        }
    ...
    private static class WrappedThreadLocal<T> {
        ...
        void set(T value) {
            OverbaardLogger.LOGGER.debug("Setting item on thread {}", Thread.currentThread().getName());
        ...
        void remove() {
            OverbaardLogger.LOGGER.debug("Removing item on thread {}", Thread.currentThread().getName());
     ...
     *
     */
    private static class OverbaardEventWrapper {
        private final boolean rerankEvent;

        private volatile OverbaardIssueEvent issueEvent;

        private volatile boolean reindexed;

        OverbaardEventWrapper(boolean rerankEvent) {
